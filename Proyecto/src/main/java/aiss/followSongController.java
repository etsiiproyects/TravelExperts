...
public class followSongController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public followSongController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if((accessTokenSpotify != null && !"".equals(accessTokenSpotify))) {
			...
			boolean success=spr.followSong(songId);
			
			if (success) {
				request.setAttribute("message", "Canción se ha comenzado a seguir");
			
			}
			else {
				request.setAttribute("message", "La canción no se ha comenzado a seguir");
			}
		}else {
